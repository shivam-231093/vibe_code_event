    ...
    <header className="flex w-full justify-evenly gap-6 items-center lg:mx- ms:mx mx- my-1">
      <BlurElement
        elements={[
          ...
          <ProfileLogo />,
          <button
            className="font-inter font-bold text-sm bg-[#FF0000CC] rounded-lg px-5 py-1.5 cursor-pointer"
            onClick={(e) => {
              navigate("/register");
            }}
          >
...
const ProfileLogo = () => {
  ...
  return (
    <>
      <div
        className="relative w-fit block ml:hidden text-white"
        onClick={() => {
          setDisplay((display) => !display);
        }}
      >
        <div className="px-2 rounded-full bg-[#009cff80] w-24 text-center">
          {currPage}
        </div>
        {display && (
          <nav
            className="absolute flex-col min-w-fit flex py-2 rounded-lg mt-0.5 z-101 w-48 -left-12 overflow-hidden bg-[#0000005b]"
      ...
      {!!display && (
        <div
          className="w-dvw h-full absolute z-100 top-0 bg-transparent left-0"
          onClick={(e) => {
            setDisplay((display) => !display);
          }}
        ></div>
